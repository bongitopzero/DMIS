...
const createNationalBudget = async (req, res) => {
  try {
    ...
    let yearValue = fiscalYear;
    if (typeof fiscalYear === 'string') {
      const parsedYear = fiscalYear.match(/\d{4}/)?.[0];
      yearValue = parsedYear ? Number(parsedYear) : Number(fiscalYear);
    }
    yearValue = Number(yearValue) || new Date().getFullYear();

    let budget = await Budget.findOne({ fiscalYear: yearValue });
    if (budget) {
      budget.allocatedBudget = numericAmount;
      budget.remainingBudget = numericAmount - (budget.spentFunds || 0);
      budget.lastUpdated = new Date();
      budget = await budget.save();
    } else {
      budget = await Budget.create({
        fiscalYear: yearValue,
        allocatedBudget: numericAmount,
        committedFunds: 0,
        spentFunds: 0,
        remainingBudget: numericAmount,
      });
    }

    res.status(200).json({ message: 'National budget saved successfully', budget });
  } catch (error) {
    console.error('Error saving national budget:', error);
...
const getEnvelopeStatus = async (req, res) => {
  try {
    const envelopes = await Budget.find().lean();
    const data = envelopes.reduce((acc, item) => {
      const key = item.fiscalYear?.toString() || 'unknown';
      acc[key] = {
        fiscalYear: item.fiscalYear,
        allocatedBudget: item.allocatedBudget || 0,
        committedFunds: item.committedFunds || 0,
        spentFunds: item.spentFunds || 0,
...
export default {
  ...
  createNationalBudget: createNationalBudget,
