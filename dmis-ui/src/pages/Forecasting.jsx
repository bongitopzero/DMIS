...
// 3. Occurrence Forecast Component
function OccurrenceForecast({ data }) {
  if (!data) {
    return (
      <section className="forecast-section">
        <p className="section-description">No occurrence forecast data available</p>
      </section>
    );
  }
    ...
    <section className="forecast-section">
      <div className="section-header">
        <div className="section-icon-title">
          ...
          <h2>Occurrence Forecast</h2>
        </div>
        <span className="section-badge primary-badge">
          {data.projectedAnnual || 0} Annual
        </span>
      </div>
      <p className="section-description">
        Projected frequency of disasters based on historical patterns
      </p>

      <div className="occurrence-stats">
        <div className="occurrence-card">
          <span className="label">Projected Annual Incidents</span>
          <span className="large-value">{data.projectedAnnual || 0}</span>
        </div>
        <div className="occurrence-card">
          <span className="label">Multi-Year Trend</span>
          <span className="trend-value">{data.trendDirection || "stable"}</span>
        </div>
      </div>

      <div className="occurrence-by-type">
        <h3>Incidents by Disaster Type</h3>
        <div className="type-forecast-grid">
          {(data.byType || []).map((type) => (
            <div key={type.type} className="type-forecast-card">
              <h4>{formatDisasterType(type.type)}</h4>
              <div className="type-forecast-stats">
                <div>
                  <span className="label">Annual:</span>
                  <span className="value">{type.projectedAnnual} incidents</span>
                </div>
                <div>
                  <span className="label">Trend:</span>
                  <span className="value">{type.trend || "stable"}</span>
                </div>
                <div>
                  <span className="label">Avg Severity:</span>
                  <span className="value">{type.avgSeverity || "medium"}</span>
                </div>
              </div>
            </div>
          ))}
